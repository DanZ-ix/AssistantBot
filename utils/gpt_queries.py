...
9. Непонятный запрос (Если ты считаешь что ни одно из дейсвтий не подходит):
   Формат: {"action": "unknown_request", "result": "Запрос пользователя"}
'''

#TODO Добавить текущую дату
...
2. Для УДАЛЕНИЯ:
   - Найди все ObjectID напоминаний, соответствующих ключевым словам и временному диапазону (если указано)
   - Не добавляй в список те напоминания, которые не подходят по времени
   - Сгенерируй фильтр с "$in": [ObjectId1, ObjectId2,...]
...
Примеры:
1. Запрос: "Добавь чипсы к напоминаниям про пиво"
   Массив содержит 3 напоминания с "пиво" в тексте с ObjectID: 111, 222, 333
   Ответ:
   {
     "operation": "update",
     "filter": {"_id": {"$in": [ObjectId('111'), ObjectId('222'), ObjectId('333')]}},
     "update": {"$set": {"reminder_text": "Открыть пиво с чипсами"}}
   }

2. Запрос: "Удалить встречи на завтра" (текущая дата: 2024-03-24)
   Найдены 2 напоминания с "встреча" в тексте на 2024-03-25 с ObjectID: 444, 555
   Ответ:
   {
     "operation": "delete",
     "filter": {"_id": {"$in": [ObjectId('444'), ObjectId('555')]}}
   }
'''
